...
function exerciseSeconds(){return DATA.exercises[exIndex].seconds||DATA.training.workSeconds}
function prepareExercise(single=true){mode=single?'single':'training';phase='work';remaining=exerciseSeconds();renderTimer();show('timer');setMusicLevel(.34);startMusic();beep(760,.1,.07);clearInterval(timer);timer=setInterval(tick,1000)}
function renderTimer(){const e=DATA.exercises[exIndex],total=exerciseSeconds();$('#timerTitle').textContent=e.title.toUpperCase();$('#roundChip').textContent=mode==='training'?`RUNDE ${round} VON ${DATA.training.rounds}`:'';$('#roundChip').style.display=mode==='training'?'inline-block':'none';$('#sideLabel').textContent=e.id==='unterarmstuetz'?'HALTEN':'SAUBER & KONTROLLIERT';$('#clock').textContent=fmt(remaining);$('#timerCue').textContent=e.timerCue;$('#timerVisual').innerHTML=e.images.map((_,n)=>imgTag(e,n,true)).join('');$('#ringBar').style.width=(remaining/total*100)+'%'}
function tick(){remaining--;renderTimer();if(remaining<=3&&remaining>0)beep(470+remaining*70,.05,.04);if(remaining<=0){clearInterval(timer);finishExercise()}}
...
function finishExercise(){beep(880,.12,.07);if(mode==='single'){setMusicLevel(.18);show('detail');return}let isLast=exIndex===DATA.exercises.length-1;if(isLast&&round>=DATA.training.rounds){setMusicLevel(.14);show('finish');return}let nextIndex=exIndex+1;if(isLast){round++;nextIndex=0}const n=DATA.exercises[nextIndex];$('#nextTitle').textContent=n.title;$('#nextTeaser').textContent=n.teaser;show('pause');setMusicLevel(.18);let p=DATA.training.pauseSeconds;$('#pauseCount').textContent=p;clearInterval(pauseTimer);pauseTimer=setInterval(()=>{p--;$('#pauseCount').textContent=Math.max(0,p);if(p<=3&&p>0)beep(460+p*60,.04,.035);if(p<=0){clearInterval(pauseTimer);exIndex=nextIndex;phase='work';remaining=exerciseSeconds();renderTimer();show('timer');setMusicLevel(.34);beep(760,.08,.05);timer=setInterval(tick,1000)}},1000)}
function startTraining(){exIndex=0;round=1;mode='training';phase='work';remaining=exerciseSeconds();renderTimer();show('timer');startMusic();setMusicLevel(.34);beep(760,.1,.07);clearInterval(timer);timer=setInterval(tick,1000)}
...
$('#skipPause').onclick=()=>{clearInterval(pauseTimer);let isLast=exIndex===DATA.exercises.length-1;let nextIndex=exIndex+1;if(isLast){round++;nextIndex=0}exIndex=nextIndex;phase='work';remaining=exerciseSeconds();renderTimer();show('timer');setMusicLevel(.34);beep(760,.08,.05);timer=setInterval(tick,1000)};
...
fetch('./training.json?v=5').then(r=>r.json()).then(d=>{DATA=d;renderGrid();updateSoundButtons()}).catch(err=>{console.error(err);document.body.insertAdjacentHTML('beforeend','<div style="padding:20px">Training konnte nicht geladen werden.</div>')});
